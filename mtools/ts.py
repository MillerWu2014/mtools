...
def selector_by_window(x_iterator: Sequence, window: int = 3, stride: int = 1) -> List:
...
def roll_slide_sample(data: np.ndarray, axis: int = 1, train_window: int = 168, predict_window: int = 168,
    ...
    for axis_of_index in selector_by_window(list(range(_shape[axis])), window=train_window + predict_window):
